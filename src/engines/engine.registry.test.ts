...
describe('engine registry', () => {
  it('exposes the four implemented engines', () => {
    ...
    expect(listEngines()).toHaveLength(4);
  });

  it('rejects unknown engine identifiers', () => {
    expect(() => getEngine('frequency-city')).toThrow(/inconnu/i);
    expect(getEngineOrDefault('frequency-city').id).toBe('minimal-album-art');
  });
});
